 ...
 * @since 6/24/2015
 */
final public class PrettyTextWriterVaers {


   static private final Logger LOGGER = Logger.getLogger( "PrettyTextWriterVaers" );
   static private final String FILE_EXTENSION = ".pretty.txt";

   private String _outputDirPath;

   public File theOutputFile = null;
   ...
   public void process( final JCas jcas ) {
      ...
      this.theOutputFile = outputFile; //get a refernce to the File to write into.
...
//      }
   ...
   public void writeFile( final JCas jCas, final String outputFilePath ) {
      try ( final BufferedWriter writer = new BufferedWriter( new FileWriter( outputFilePath ) ) ) {
         ...
         final Collection<Sentence> sentences = JCasUtil.select( jCas, Sentence.class );
         if(sentences.size() == 0){
        	 //Try another type system
        	 Collection<org.cleartk.token.type.Sentence> sentencesClearTk = JCasUtil.select( jCas, org.cleartk.token.type.Sentence.class );
             for ( org.cleartk.token.type.Sentence sentence : sentencesClearTk ) {
                 writeSentence( jCas, sentence, writer );
              }
         }
         else{

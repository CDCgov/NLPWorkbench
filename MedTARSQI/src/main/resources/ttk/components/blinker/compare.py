...
def test_compare_date(test_string):
    (date1, date2, creation_year_int) = test_string.split()
    return compare_date(date1, date2, creation_year_int)
        ...
        if (len(year1) == len(year2)):
            ...
            if (need_compare_at_year_boundary(year1, year2, month1, month2, day1, day2)):
                if (re.match(r'W', month2)):
                    link = compare_month_week_at_year_boundary(
                        int(year1), int(year2),
                        int(month1), int(strip_week(month2)),
                        int(day1), int(day2))
                    return link
                elif(re.match(r'W', month1)):
                    link = compare_month_week_at_year_boundary(
                        int(year2), int(year1),
                        int(month2), int(strip_week(month1)),
                        int(day2), int(day1))
            ...
            if (year1_int < year2_int):  # and
                ...
                return "NEED_COMPARE_IN_YEAR"

        else:
            return compare_short_year(year1, year2)
...
def compare_in_year():
    ...
    elif((month1_type == "MONTH" and month2_type == "WEEK") or
...
def compare_week_month_aux(week, month, week_day, month_day):
    # year is an integer, week is string, month is string, week_day is string,
    # month_day is string
    week = strip_week(week)
    week_int = int(week)
    month_int = int(month)
...
def compare_week_month_aux_aux(week_int, month_int, week_day_int, month_day_int):
    if (month_day_int == 0):
        ...
        if (calendar.isleap(int(year1)) and month_int == 2):
            ...
            month_date_end = datetime.date(int(year1), month_int, month_days[month_int])

        month_to_week_begin = month_date_begin.isocalendar()
        month_to_week_end = month_date_end.isocalendar()

        # Here should be very careful as a month date when converted to week, it
        # may fall into the week in the year before or it may also fall into the
        # week in the year after

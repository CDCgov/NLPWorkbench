...
public class TestETHERRelation {
	...
	public void test() throws IOException, JDOMException, CASException, InvalidXMLException, ResourceInitializationException {
		...
		for (String s : testStrings) {
			...
			/**
			 * IMPORTANTLY, the output of all the processETHER methods has changed (as of 2018-08-30).
			 * They now return the name of the ETHERNLP output file, rather than the full contents of that file.
			 */
			String clinicalResultFilename = etherVar.processETHERClinical(s);
			
			String temporalResultFilename = etherVar.processETHERTemporal(s);
			
			String relationResultFilename = etherVar.processETHERRelation();
			
			try {
				// The following few lines will produce a CAS object containing the information from the output file.
				
				VAERSParser parser = new VAERSParser(relationResultFilename);

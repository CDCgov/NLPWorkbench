This project provides a convenient API for developers to gain the functionalities of ETHER Temporal/Relation.
...
2. Calling the function to obtain result: the following function will accept a String containing the raw text,
 and generates a String containing the filename for the XML content of the temporal and relation result conforming to the VAERS Type System.
 ...
 *** Importantly, clinical result and temporal result are precondition of obtaining the relation result. ***
		...
		String clinicalResultFilename =
				etherVar.processETHERClinical(s);

		String temporalResultFilename =
				etherVar.processETHERTemporal(s);
		
		String relationResultFilename =
				etherVar.processETHERRelation();
		
3. Clean up temporary file(s): some temporary files will be removed accordingly.

		etherVar.cleanUpFiles();
		
		"etherVar.cleanUpFiles()" will remove all the temporary files for clinical, temporal and relation processing.

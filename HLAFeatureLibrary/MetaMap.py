...
def removeNonAscii(s,replacement = None):
        ...
        for char in s :
            if ord(char) >= 128:
                if replacement == None:
                    ...
                    cleaned_text = cleaned_text+ replacement
            else:
...
def load_snomed():
    data = {}
    for line in open('data/SCT_20171031AU/descriptions').readlines()[1:]:
        line = line[:-1]
        cid = line.split('\t')[4]
        des = line.split('\t')[7]
        des = removeNonAscii(des)
        data[cid] = des
    return data

def find_snomed(snomed_data, term):
    match_code = []
    local = time.time()
    for code, description in snomed_data.items():

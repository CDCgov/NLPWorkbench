...
class FeatureExtractor:
    ...
    def __init__(self, filename="", lif_string=""):
        ...
        if lif_string != "":
            print("Loading LIf FILE")
            self.lif_loader = LifFileParser(string=lif_string)
            print("finish loading lif file")
    ...
    def extract_tokens(self, time_feature = False):
        ...
        print("Finish extracting tokens")
                ...
                if ann['label'] == 'Token':
                    ...
                    if 'pos' in ann['features'].keys():
                        ...
                        if key in self.token_features.keys():
                            ...
                            self.token_features[key]['pos'] = 'UN'
                    else:
                        self.token_features[key]['pos'] = 'UN'
            ...
            for ann in annotations:
                if ann['label'] == 'Token':
                    ...
                    if 'orth' in ann['features'].keys():
                        ...
                        self.token_features[key]['orth'] = orth
                        # self.token_features[key].append(orth)
                    else:
                        self.token_features[key]['orth'] = 'UN'
            ...
            for ann in annotations:
                if ann['label'] == 'Token':
                    ...
                    if 'tokenType' in ann['features'].keys():
                        types = ann['features']['tokenType']
                        self.token_features[key]['type'] = types
                    else:
                        self.token_features[key]['type'] = 'UN'
        ...
        if annotations == []:
            ...
            for ann in annotations:
                if ann['label'] == 'Token':
                    ...
                    key = ann['start']
                    if key < self.start_position and key not in self.token_features.keys():
                        continue
    ...
    def extract_code_section(self):
        ...
        for ann in annotations:
            if ann['label'] == "Sentence":
                ...
                if camal == True:
                    code, name = find_snomed(snomed_data, sent)
    ...
    def extract_code_chunk(self):
        ...
        for (key, value) in chunks.items():
            if '.' not in str(value[0]) and str(value[0]).isalpha() and str(value[0]) not in stopwords and len(str(value[0])) > 1:
                code, name = find_snomed(snomed_data, str(value[0]))
    ...
    def extract_code_verb(self):
        ...
        for ann in annotations:
            if ann['label'] == 'VG':
                ...
                code, name = find_snomed(snomed_data, phrase)
                key = str(start) + ' ' + str(end)

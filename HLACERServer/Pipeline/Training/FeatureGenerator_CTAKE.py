...
class cTakeFeatureGenerator:
	...
	def __init__(self, input_string, position_filename, bio_filename):
	...
	def save_position(self):
		...
		for start, item in self.feature_data.items():
			...
			if (word == 'Gross' or word == 'Final' or word == 'Nature' or word == 'Path' or word == "PATH" or word == "DIAGNOSIS" or word == "Diagnosis") and not start_tag:
				start_tag = True
			if start_tag:
				if word not in ['NEWLINE', 'SPACE', 'TAB']:
					line = word + ' ' + str(start) + '\n'
					position_file.write(line)
				if start in self.sentence:
					bio_file.write("\n")

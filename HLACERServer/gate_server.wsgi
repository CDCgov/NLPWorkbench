...
import bottle
from Pipeline.Training.GATE import *
...
def generate_bio(transaction_id, document_text, file_num):
	...
	temp_bio_file = 'temp/bio/gate/train/' + file_num + '.bio'
	temp_pos_file = 'temp/bio/gate/tagged/' + file_num + '.pos'
	...
	feature_extractor(result_lif, 2, 2, temp_bio_file, temp_pos_file)
		

def classify(transaction_id, document_text, file_num):
	service_log_file = 'data/gate_service_call.log'
	...
	try:
		...
		temp_bio_file = 'temp/bio/gate/train/' + file_num + '.bio'
		temp_pos_file = 'temp/bio/gate/tagged/' + file_num + '.pos'
		temp_output_file = 'temp/bio/gate/tagged/' + file_num + '.bio'
		output_file_bio = 'temp/bio/gate/' + file_num + '.bio'
		output_file = 'temp/output/gate/' + file_num + '.txt'
		MODEL_NAME = 'temp/input/gate_model'

		# Run through the pipeline
		crf_runner = CRFRunner(model_file=MODEL_NAME, source="gate")
		...
		log = transaction_id + " || " + "GATE Pipeline || " + time.asctime( time.localtime(time.time())) + " || SUCCESSFULL" + "\n"
		...
		log = transaction_id + " || " + "GATE Pipeline || " + time.asctime( time.localtime(time.time())) + " || " + message + "\n"
		server_log.write(log)
...
def output_json(output_filename, document_text):
	...
	view = {}
	view['metadata'] = {}
	view['metadata']['contains'] = 'GATE Pipeline Service'
	view['annotation'] = []
	id = 0
	for line in output_file.readlines():
		...
		ann['features'] = {}
		ann['features']['annotator'] = 'GATE_MODEL'
...
@app.route('/request_cdc_gate_service', method='POST')
def generate_stanford_result():
	...
	try:
		...
		traceback.print_exc()
		status = "FAILED"
	with open('data/gate_server.log', 'a') as log_file:
...
@app.route('/fetch_cdc_gate_service_result/:transaction_id', method='GET')
def fetch_stanford_result(transaction_id):
	log_file = open('data/gate_server.log')
...
def find_error_message(transaction_id):
	error_file = open('data/gate_service_call.log')

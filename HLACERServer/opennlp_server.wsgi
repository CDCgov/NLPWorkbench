...
import bottle
from Pipeline.Training.OpenNLP import *
...
def generate_bio(transaction_id, document_text, file_num):
	...
	temp_bio_file = 'temp/bio/opennlp/train/' + file_num + '.bio'
	temp_pos_file = 'temp/bio/opennlp/tagged/' + file_num + '.pos'
	...
	tokenizer_lif = ""
	try:
		tokenizer_lif = tokenizer(post_sentence_lif)
	except:
		tokenizer_lif = opennlp_tokenizer(post_sentence_lif)
...
def classify(transaction_id, document_text, file_num):
	service_log_file = 'data/opennlp_service_call.log'
	...
	try:
		...
		temp_bio_file = 'temp/bio/opennlp/train/' + file_num + '.bio'
		temp_pos_file = 'temp/bio/opennlp/tagged/' + file_num + '.pos'
		temp_output_file = 'temp/bio/opennlp/tagged/' + file_num + '.bio'
		output_file_bio = 'temp/bio/opennlp/' + file_num + '.bio'
		output_file = 'temp/output/opennlp/' + file_num + '.txt'
		MODEL_NAME = 'temp/input/opennlp_model'

		# Run through the pipeline
		crf_runner = CRFRunner(model_file=MODEL_NAME, source="opennlp")
		...
		log = transaction_id + " || " + "OPENNLP Pipeline || " + time.asctime( time.localtime(time.time())) + " || SUCCESSFUL" + "\n"
		...
		log = transaction_id + " || " + "OPENNLP Pipeline || " + time.asctime( time.localtime(time.time())) + " || " + message + "\n"
		server_log.write(log)
...
def output_json(output_filename, document_text):
	...
	view = {}
	view['metadata'] = {}
	view['metadata']['contains'] = 'OpenNLP Pipeline Service'
	view['annotation'] = []
	id = 0
	for line in output_file.readlines():
		...
		ann['features'] = {}
		ann['features']['annotator'] = 'OPENNLP_MODEL'
...
@app.route('/request_cdc_opennlp_service', method='POST')
def generate_stanford_result():
	...
	try:
		...
		traceback.print_exc()
		status = "FAILED"
	with open('data/opennlp_server.log', 'a') as log_file:
...
@app.route('/fetch_cdc_opennlp_service_result/:transaction_id', method='GET')
def fetch_stanford_result(transaction_id):
	log_file = open('data/opennlp_server.log')
...
def find_error_message(transaction_id):
	error_file = open('data/opennlp_service_call.log')
